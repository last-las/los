...
fn test_heap_oom_rescue() {
    static mut SPACE: [usize; 1000] = [0; 1000];
    // statement
    let heap = LockedHeap::new(|heap: &mut Heap| unsafe {
        heap.add_to_heap(SPACE.as_ptr().add(100) as usize, SPACE.as_ptr().add(700) as usize);
    });
    // add to heap
    unsafe {
        heap.lock().add_to_heap(SPACE.as_ptr() as usize, SPACE.as_ptr().add(100) as usize);
    }

    unsafe {
        assert_ne!(heap.alloc(Layout::from_size_align(100 * size_of::<usize>(), 1).unwrap()) as usize, 0);
    }
}
