...
pub fn read_block(block_id: usize, buf: &mut [u8]) {
    SDCARD_WRAPPER.read_block(block_id, buf);
}

pub fn write_block(block_id: usize, buf: &[u8]) {
    SDCARD_WRAPPER.write_block(block_id, buf);
}

lazy_static!{
    pub static ref SDCARD_WRAPPER: Arc<SDCardWrapper> = Arc::new(SDCardWrapper::new());
}
